document.addEventListener('DOMContentLoaded', function () {
    ...
    const table = document.getElementById('reserveTable');  // Référence au tableau actuel

    searchInput.addEventListener('input', function () {
        const query = this.value.trim();

        fetch('index.php?controller=Admin&action=orderReservations', {
            method: 'POST',
            headers: {
            ...
            .then(html => {
                ...
                const newTableBody = tempDiv.querySelector('#reserveTable tbody');
                if (newTableBody) {
