document.addEventListener('DOMContentLoaded', function () {
    ...
    const table = document.getElementById('demandeTable');  // Référence au tableau actuel

    searchInput.addEventListener('input', function () {
        const query = this.value.trim();

        fetch('index.php?controller=Admin&action=orderDemande', {
            method: 'POST',
            headers: {
            ...
            .then(html => {
                ...
                const newTableBody = tempDiv.querySelector('#demandeTable tbody');
                if (newTableBody) {
