document.addEventListener('DOMContentLoaded', function () {
    ...
    const table = document.getElementById('archiveTable');  // Référence au tableau actuel

    searchInput.addEventListener('input', function () {
        const query = this.value.trim();

        fetch('index.php?controller=Admin&action=orderArchives', {
            method: 'POST',
            headers: {
            ...
            .then(html => {
                ...
                const newTableBody = tempDiv.querySelector('#archiveTable tbody');
                if (newTableBody) {
