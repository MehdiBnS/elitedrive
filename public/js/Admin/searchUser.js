document.addEventListener('DOMContentLoaded', function () {
    ...
    const table = document.getElementById('userTable');  // Référence au tableau actuel

    searchInput.addEventListener('input', function () {
        const query = this.value.trim();

        fetch('index.php?controller=Admin&action=orderUsers', {
            method: 'POST',
            headers: {
            ...
            .then(html => {
                ...
                const newTableBody = tempDiv.querySelector('#userTable tbody');
                if (newTableBody) {
