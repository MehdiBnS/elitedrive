document.addEventListener('DOMContentLoaded', function () {
    ...
    const table = document.getElementById('avisTable');  // Référence au tableau actuel

    searchInput.addEventListener('input', function () {
        const query = this.value.trim();

        fetch('index.php?controller=Admin&action=orderAvis', {
            method: 'POST',
            headers: {
            ...
            .then(html => {
                ...
                const newTableBody = tempDiv.querySelector('#avisTable tbody');
                if (newTableBody) {
