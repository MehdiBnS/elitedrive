    ...
    document.querySelectorAll('.modifier').forEach(btn => {
        btn.addEventListener('click', function (e) {
            e.preventDefault();
            const li = btn.closest('.avis-item');
            ...
            const commentaire = li.innerText.split(':')[1].trim();
    ...
    document.getElementById('formUpdateAvis').addEventListener('submit', function (e) {
        ...
        .then(response => {
            ...
            location.reload(); // ou tu peux mettre à jour le DOM sans reload si tu veux aller plus loin
        })
        .catch(error => {
